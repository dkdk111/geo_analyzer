...
# 모든 아이콘/이미지/배경색 제거 및 폰트 설정 (Pretendard)
st.markdown("""
    <link rel="stylesheet" as="style" crossorigin href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/static/pretendard.css" />
    <style>
        * { font-family: 'Pretendard', sans-serif !important; }
        
        /* 모든 아이콘 및 아이콘 텍스트 완전 소멸 */
...
with st.sidebar:
    ...
    # 관리자 메뉴 (사이드바 최하단 배치)
    for _ in range(15): st.write("") # 간격 조절
    st.divider()
    with st.expander("관리자 메뉴"):
        admin_pw = st.text_input("비밀번호", type="password")
        if admin_pw == "1213": # 비밀번호 설정
            try:
                conn = sqlite3.connect("inquiries.db")
                df = pd.read_sql_query("SELECT * FROM inquiries ORDER BY timestamp DESC", conn)
                st.dataframe(df)
                csv = df.to_csv(index=False).encode('utf-8-sig')
                st.download_button("명단 다운로드 (CSV)", data=csv, file_name="inquiries.csv", mime="text/csv")
                conn.close()
            except:
                st.text("신청 데이터가 없습니다.")
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                with slot.container():
                    ana = res["analysis"]
                    v_html = '<span class="found">발견됨</span>' if ana['mentioned'] else '<span class="not-found">미발견</span>'
                    st.markdown(f"결과: {v_html} | 적합도: {ana['solution_fit']}/10", unsafe_allow_html=True)
...
if st.session_state.report:
    ...
    with st.form("inquiry_form"):
        ...
        if st.form_submit_button("상담 신청하기", type="primary", use_container_width=True):
            if name and contact:
                engine = GEOAnalyzer()
                engine.save_inquiry(st.session_state.target_brand, st.session_state.target_keyword, name, contact, msg)
