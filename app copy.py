...
# 모든 아이콘/이미지/Material Text 완전 소멸 CSS
st.markdown("""
...
if analyze_btn and brand_in and kw_in:
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                with slot.container():
                    ana = res["analysis"]
                    st.text(f"결과: {'발견됨' if ana['mentioned'] else '미발견'} | 적합도: {ana['solution_fit']}/10")
                    st.markdown(f'<div class="answer-box">{highlight(res["raw"], brand_in)}</div>', unsafe_allow_html=True)
                    st.caption(f"진단 근거: {ana.get('fit_reason', '')}")
    ...
    with st.spinner("종합 진단 보고서를 작성 중입니다..."):
    ...
    st.text("위 진단 결과는 귀사의 브랜드가 AI 검색 시장에서 마주한 실제 가시성입니다.")
    
    with st.form("inquiry_form"):
        name = st.text_input("성함 / 기업명")
        contact = st.text_input("연락처 (이메일 혹은 휴대전화)")
        msg = st.text_area("상담 희망 내용")
        if st.form_submit_button("상담 신청 및 전략 리포트 예약", type="primary", use_container_width=True):
            if name and contact:
                engine.save_inquiry(brand_in, kw_in, name, contact, msg)
                st.text("상담 신청이 완료되었습니다. 담당 전문가가 곧 연락드리겠습니다.")
            else:
                st.text("정보를 모두 입력해주세요.")